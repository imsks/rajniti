...
@api_bp.route('/elections/<election_id>/constituencies/<constituency_id>/candidates', methods=['GET'])
def get_candidates_by_constituency(election_id, constituency_id):
    """Get candidates by constituency"""
    try:
        results = candidate_controller.get_candidates_by_constituency(constituency_id, election_id)
        
        if not results:
            return jsonify({
                'success': False,
                'error': 'Election or constituency not found'
            }), 404
...
@api_bp.route('/elections/<election_id>/constituencies/<constituency_id>', methods=['GET'])
def get_constituency(election_id, constituency_id):
    """Get constituency details"""
    try:
        results = constituency_controller.get_constituency_by_id(constituency_id, election_id)
...
@api_bp.route('/elections/<election_id>/constituencies/<constituency_id>/results', methods=['GET'])
def get_constituency_results(election_id, constituency_id):
    """Get constituency results"""
    try:
        results = constituency_controller.get_constituency_results(constituency_id, election_id)
