...
class JsonDataService(DataService):
    ...
    def get_candidates(self, election_id: str) -> List[Dict[str, Any]]:
        ...
        if election.type == ElectionType.LOK_SABHA:
            file_path = self.data_root / "lok_sabha" / f"{election_id}" / "candidates.json"
        else:
            if election.state_code == "DL":
                file_path = self.data_root / "vidhan_sabha" / "DL_2025_ASSEMBLY" / "candidates.json"
            elif election.state_code == "MH": 
                file_path = self.data_root / "vidhan_sabha" / "MH_2024" / "candidates.json"
            else:
                return []
    ...
    def get_parties(self, election_id: str) -> List[Party]:
        ...
        if election.type == ElectionType.LOK_SABHA:
            ...
            if election.state_code == "DL":
                file_path = self.data_root / "vidhan_sabha" / "DL_2025_ASSEMBLY" / "parties.json"
            elif election.state_code == "MH":
    ...
    def get_constituencies(self, election_id: str) -> List[Constituency]:
        ...
        if election.type == ElectionType.LOK_SABHA:
            file_path = self.data_root / "lok_sabha" / f"{election_id}" / "constituencies.json"
        else:
            if election.state_code == "DL":
                file_path = self.data_root / "vidhan_sabha" / "DL_2025_ASSEMBLY" / "constituencies.json"
            elif election.state_code == "MH":
                file_path = self.data_root / "vidhan_sabha" / "MH_2024" / "constituencies.json"
            else:
                return []
    ...
    def search_candidates(self, query: str, election_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """Search candidates by name, party, or constituency"""
        results = []
        elections = [self.get_election(election_id)] if election_id else self.get_elections()
        ...
        for election in elections:
            ...
            for candidate in candidates:
                ...
                constituency_field = candidate.get('constituency', '') or candidate.get('Constituency Code', '')
                
                if (query_lower in name_field.lower() or 
                    query_lower in party_field.lower() or
                    query_lower in constituency_field.lower()):
    ...
    def get_candidate_by_id(self, candidate_id: str, election_id: str) -> Optional[Dict[str, Any]]:
    ...
    def get_constituency_by_id(self, constituency_id: str, election_id: str) -> Optional[Constituency]:
